...
export default function App() {
  ...
  return (
    <div className="app-shell">
      ...
      {missingEnv.length > 0 && (
        <div className="shell connect-error">
          <p role="status" className="testnet-notice">
      ...
      <main id="main" tabIndex={-1}>
        ...
        {view === 'vault' && <>
          ...
          <div className="shell trading-content"><div className="testnet-notice" role="status">{c('Testnet environment. Use test assets only — bank transfers are simulated.', 'Testnet ortamı. Yalnızca test varlıkları kullanın — banka transferleri simüle edilir.')}</div><PoolView address={session?.address ?? null} onConnect={() => void onConnect()} connecting={connecting}/></div>
        </>}
