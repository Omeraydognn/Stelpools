//! The share as a SEP-41 token.
//!
//! A pool position that cannot move is only half a position. Implementing the
//! standard token interface makes vault shares transferable, spendable
...
pub fn name(e: &Env) -> String {
    String::from_str(e, "USDC Vault Share")
}

pub fn symbol(e: &Env) -> String {
    String::from_str(e, "vUSDC")
}
...
/// Destroy shares. The caller must already have checked authorisation.
...
/// Burn without withdrawing: the assets behind these shares stay in the
/// vault, so every remaining share becomes worth more.
